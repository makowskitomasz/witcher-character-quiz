...
class GraphVisualizer:
    ...
    def visualize_multi_panel(
        ...
        plt.suptitle(
            "Witcher Knowledge Graph Visualization",
            fontsize=16,
            fontweight="bold",
            y=0.98,
        )
    ...
    def _plot_full_graph(self, ax) -> None:
        ...
        factions = [
            n
            for n, attrs in self.graph.nodes(data=True)
            if attrs.get("type") == "Faction"
        ]
        traits = [
            n
            for n, attrs in self.graph.nodes(data=True)
            if attrs.get("type") == "TraitDimension"
        ]
        ...
        char_char_edges = [
            ...
            in ["friend_of", "lover_of", "destiny_of", "ward_of", "adoptive_child_of"]
        ]
        faction_edges = [
            (u, v)
            for u, v, d in self.graph.edges(data=True)
        ...
        char_faction_labels = {
            n: self.graph.nodes[n].get("name", n) for n in characters + factions
        }
    ...
    def _plot_trait_heatmap(self, ax) -> None:
        ...
        ax.set_title(
            "Character Trait Heatmap (Clustered)", fontsize=12, fontweight="bold"
        )
