...
class EmbeddingQualityAnalyzer:
    ...
    def reduce_dimensions(self) -> np.ndarray:
        ...
        embeddings_matrix = np.array(
            [self.character_embeddings[cid] for cid in char_ids]
        )
            ...
            for char_id in char_ids:
                ...
                if factions:
                    ...
                    if faction_id not in faction_colors:
                        faction_colors[faction_id] = color_map(
                            faction_idx / max(1, len(faction_colors))
                        )
                        faction_idx += 1
    ...
    def analyze_clustering(self, n_clusters: int | None = None) -> Dict[str, any]:
        ...
        return {
            ...
            "cluster_assignments": {
                cid: int(cluster_labels[char_ids.index(cid)]) for cid in char_ids
            },
